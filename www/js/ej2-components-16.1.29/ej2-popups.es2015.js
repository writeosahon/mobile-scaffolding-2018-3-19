import { Animation, Browser, ChildProperty, Collection, Complex, Component, Draggable, Event, EventHandler, L10n, NotifyPropertyChanges, Property, Touch, addClass, attributes, classList, closest, createElement, detach, formatUnit, getUniqueID, isNullOrUndefined, prepend, remove, removeClass, setStyleAttribute } from '@syncfusion/ej2-base';
...
function setPopup(element, pos) {
    ...
    if (element.offsetParent != null
        ...
        let data = calculatePosition(element.offsetParent, 'left', 'top');
        left = data.left;
        top = data.top;
    }
...
let Popup = class Popup extends Component {
    ...
    getRelateToElement() {
        let relateToElement = this.relateTo === '' ? document.body : this.relateTo;
    ...
    isElementVisible(relateToElement, scrollElement) {
        let rect = relateToElement.getBoundingClientRect();
    ...
    getAnchorPosition(anchorEle, ele, position, offsetX, offsetY) {
        ...
        let anchorRect = anchorEle.getBoundingClientRect();
...
function getZindexPartial(element) {
    ...
    let finalValue = parentZindex.concat(childrenZindex);
...
let Dialog = class Dialog extends Component {
    ...
    setOverlayZindex() {
        let zIndex = parseInt(this.element.style.zIndex, 10) ? parseInt(this.element.style.zIndex, 10) : this.zIndex;
    ...
    setTargetContent() {
        ...
        if (this.element.children.length > 0 || isContent) {
            let contentDiv = document.createDocumentFragment();
...
let Tooltip = class Tooltip extends Component {
    ...
    renderCloseIcon() {
        ...
        let tipClose = createElement('div', { className: ICON$1 + ' ' + CLOSE });
        ...
        for (let prop of Object.keys(newProp)) {
            switch (prop) {
                ...
                case 'tipPointerPosition':
                    let trgt = document.querySelector('[data-tooltip-id= ' + this.ctrlId + '_content]');
...
function createSpinner(args) {
    let radius;
    let container = create_spinner_container(args.target);
...
function create_spinner_container(target) {
    ...
    let spinnerInnerContainer = document.createElement('div');
...
export { PositionData, Popup, getScrollableParent, getZindexPartial, calculateRelativeBasedPosition, calculatePosition, fit, isCollide, flip, ButtonProps, AnimationSettings, Dialog, Animation$1 as Animation, Tooltip, createSpinner, showSpinner, hideSpinner, setSpinner };
